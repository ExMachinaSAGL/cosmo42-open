...
class ChatResponseDtoContractTest {
    ...
    void uuidEventSerializesWithStringTypeAndUuidPayload() throws IOException {
        ...
        JsonNode node = json.write(dto).getJson() != null ? readTree(dto) : null;
    ...
    void declaredEventTypesPinsTheContractAgainstAccidentalAddition() {
        // If a new event type is added to the enum, this test must be updated AND the
        // shape of the new event documented above. The intent is to make the contract
        // change visible in a PR review.
